...
| --- | --- | --- |
| Validation provenance | recorded | Startup artwork validation covered `3d5fc88` (`feat: render startup artwork`) and `5bc6c6f` (`fix: preserve startup artwork frame`), with final behavioral code at `5bc6c6f`. The complete cited command set was rerun at `5bc6c6f`; this later documentation-only commit records that provenance without changing runtime code, fixtures, or tests. Existing Task3 records remain historical records of their respective validation runs and are not rewritten here. Every cited command exited 0. |
