...
The **startup artwork viewport** uses the literal `ascii-art.txt` design when the live region can fit all 69 display columns and 15 rows. Its Version, Project, and Tools slots use the live package version, working-directory component, and active tool count. Values are clipped at grapheme boundaries with `...` and padded to their fixed slots; unavailable Project or Tools values leave their slots blank. Model and Skills are intentionally absent from this fixed artwork. When the live region cannot fit the complete artwork, startup metadata retains the existing responsive compact rendering, including omission behavior and the width-3 `Picopilot` fallback. Exactly one blank row MUST separate the visible startup surface from the prompt.
...
- [x] Verify transcript rendering has no secondary `Paragraph::wrap` or horizontal transcript padding through the production transcript-to-buffer path; `unstable-rendered-line-info` remains dependency-unverified.
- [x] Verify every one of the 69 palette keys and exact RGB values.
- [x] Verify the shared 50 ms tick contract drives spinner and tool phases from one timestamp and stops redraw/ticking when idle; reduced motion and idle have no independent animation advancement path.
...
- [x] Verify diff timeout, 3-line context, two-counter numbering, wrapped repeated sigils, 0.4 word threshold, and no row cap.
